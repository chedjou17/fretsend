...
function buildTrackingNumber(country) {
    const now = new Date();
...
let PackagesService = PackagesService_1 = class PackagesService {
    constructor(db, notifications, emailService, trackingGateway) {
        this.db = db;
    ...
    async findAll(query, userId, userRole, userAgencyId) {
        ...
        else if (['agent', 'agency_manager'].includes(userRole) && userAgencyId)
            q = q.or(`origin_agency_id.eq.${userAgencyId},destination_agency_id.eq.${userAgencyId},current_agency_id.eq.${userAgencyId}`);
        if (search)
            q = q.or(`tracking_number.ilike.%${search}%,sender_name.ilike.%${search}%,recipient_name.ilike.%${search}%,sender_phone.ilike.%${search}%,recipient_phone.ilike.%${search}%`);
        if (status)
            q = q.eq('status', status);
        if (transport_type)
            q = q.eq('transport_type', transport_type);
        if (agency_id)
            q = q.or(`origin_agency_id.eq.${agency_id},destination_agency_id.eq.${agency_id},current_agency_id.eq.${agency_id}`);
        if (date_from)
    ...
    async findByTracking(trackingNumber) {
        ...
        if (error || !pkg)
            throw new common_1.NotFoundException(`Colis "${trackingNumber}" introuvable`);
    ...
    async findById(id, userId, userRole) {
        ...
        if (userRole === 'client' && pkg.sender_id !== userId)
    ...
    async checkUserByEmail(email) {
        ...
        if (data)
            return { found: true, id: data.id, full_name: `${data.first_name} ${data.last_name}` };
        return { found: false };
    }
    async create(dto, userId) {
        ...
        if (!originAgency?.is_active)
            throw new common_1.BadRequestException('Agence d\'origine invalide ou inactive');
        ...
        if (!destAgency?.is_active)
            throw new common_1.BadRequestException('Agence de destination invalide ou inactive');
        if (originAgency.country === destAgency.country)
            throw new common_1.BadRequestException('L\'envoi doit être international : France ↔ Cameroun');
        ...
        if (dto.is_urgent)
            price *= 1.5;
        if (dto.is_fragile)
            price += originAgency.country === 'FR' ? 5 : 500;
        if (dto.is_insured && dto.declared_value)
            price += dto.declared_value * 0.02;
        price = Math.round(price * 100) / 100;
        const eta = new Date();
        eta.setDate(eta.getDate() + (dto.transport_type === 'air' ? 5 : 25));
    ...
    async handlePostCreation(pkg, dto) {
        const tempPasswords = [];
        ...
        if (fullPkg && dto.sender_email)
            await this.emailService.sendPaymentLink(fullPkg, paymentToken);
    }
    async updateStatus(id, dto, userId) {
        ...
        if (dto.agency_id)
            updates.current_agency_id = dto.agency_id;
        ...
        if (fullPkg) {
            if (dto.status === 'available')
                this.emailService.sendAvailableNotification(fullPkg).catch(e => this.logger.warn(e.message));
            else if (['in_transit', 'at_customs', 'arrived_hub', 'dispatched', 'delivered', 'returned'].includes(dto.status))
                this.emailService.sendStatusUpdate(fullPkg, dto.status, STATUS_TITLES[dto.status], dto.note || STATUS_TITLES[dto.status]).catch(e => this.logger.warn(e.message));
        }
        this.logger.log(`Statut : ${pkg.tracking_number} → ${dto.status}`);
        return { message: 'Statut mis à jour' };
    }
    async getStats(userRole, agencyId) {
        let q = this.db.from('packages').select('status,transport_type,created_at,price,currency');
        if (['agent', 'agency_manager'].includes(userRole) && agencyId)
            q = q.or(`origin_agency_id.eq.${agencyId},destination_agency_id.eq.${agencyId},current_agency_id.eq.${agencyId}`);
        const { data } = await q;
        if (!data)
            return {};
        ...
        return {
            ...
            by_transport: data.reduce((acc, p) => { acc[p.transport_type] = (acc[p.transport_type] || 0) + 1; return acc; }, {}),
